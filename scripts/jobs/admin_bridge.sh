...
BRIDGE_ACTIVATION_ID=$(az resource list \
      ...
      | jq -r ".[0].id") \
  && azs_log_field T status admin_bridge_activation_id \
  || azs_log_field T status admin_bridge_activation_id fail
...
BRIDGE_REGISTRATION_ID=$(az resource show \
  ...
  && azs_log_field T status admin_bridge_registration_id \
  || azs_log_field T status admin_bridge_registration_id fail

# Remove leading "/"
BRIDGE_SUBSCRIPTION_ID=${BRIDGE_REGISTRATION_ID#*/} \
  && azs_log_field T status admin_bridge_substring_01 \
  || azs_log_field T status admin_bridge_substring_01 fail

# Remove "sbscriptions/"
BRIDGE_SUBSCRIPTION_ID=${BRIDGE_SUBSCRIPTION_ID#*/} \
  && azs_log_field T status admin_bridge_substring_02 \
  || azs_log_field T status admin_bridge_substring_02 fail

# Remove traling path from subscription id
BRIDGE_SUBSCRIPTION_ID=${BRIDGE_SUBSCRIPTION_ID%%/*} \
  && azs_log_field T status admin_bridge_substring_03 \
  || azs_log_field T status admin_bridge_substring_03 fail

echo $BRIDGE_SUBSCRIPTION_ID > /azs/bridge/subscriptionid \
  && azs_log_field T status admin_bridge_to_file \
  || azs_log_field T status admin_bridge_to_file fail
